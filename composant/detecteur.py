...
class Detecteur:
    ...
    def detecte(self):
        ...
        mini = 1000000 # Valeur par défault
        ...
        # Si le segment n'est pas dans l'axe x, on trouve l'équation de droite de 
        # type a1*x + b1
        ...
        for o in obj: # On iter sur tous les objets de l'aréne
            if (o == self.robot): # Le robot ne doit pas se detecter pas lui même
                continue
            ...
            for j in range(len(opt)): # On iter sur tous les points de l'objet
                ...
                # le robot est dans l'axe x, trouve le pts d'intersection dans ce cas
                if (a1 == None) : 
                    x = b1
                    y = a2*x + b2
                
                # le segment est dans l'axe x, trouve le pts d'intersection dans ce cas
                elif (a2 == None):
                    x = b2
                    y = a1*x + b1
                    
                # Trouve le pts d'intersection des deux droites
                else :
                    x = (b2 - b1) / (a1 - a2)
                    y = a1*x + b1
                ...
                if mmsigne(p2[0] - p1[0],x - p1[0]) and mmsigne(p2[1] - p1[1],y - p1[1]):
                    ...
                    # On cherche à savoir si le pts d'intersection est sur le segment
                    if (min(p3[0],p4[0])<=x<=max(p3[0],p4[0]) and 
                        min(p3[1],p4[1])<=y<=max(p3[1],p4[1])):
                        
                        
                        # On regarde si l' objet est à la bonne hauteur pour être detecter
                        ...
                        # que toute les longueurs mesurer jusqu'à présent
        ...
        # Si cet objet n'existe pas, renvoie la valeur par défault 1000000
        return mini 
